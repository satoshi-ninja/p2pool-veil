    ...
    try:
        ...
        if args.pubkey_hash is None and args.address != 'dynamic':
            ...
            if address is not None:
                ...
                res2 = yield deferral.retry('Error getting cached address info:', 5)(lambda: bitcoind.rpc_getaddressinfo(address))()
                if not res['isvalid'] or not res2['ismine']:
                    print '    Cached address is either invalid or not controlled by local bitcoind!'
                    address = None
            
            if address is None:
                print '    Getting payout address from bitcoind...'
                address = yield deferral.retry('Error getting payout address from bitcoind:', 5)(lambda: bitcoind.rpc_getnewminingaddress())()
